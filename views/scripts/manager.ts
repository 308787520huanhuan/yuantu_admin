...
class ViewManager {
    ...
    constructor() {
        ...
        // this.platform_info_db = new db.Pool(this.config.platform_info);
        // this.platform_db = new db.Pool(this.config.db);
        this.yuantu_db = new db.Pool(this.config.yuantu_db);
        this.nameMap = new Map<string, string>();
        let ps = JSON.parse(fs.readFileSync("views/platforms.json",'utf-8'));
        for(let p of ps)
            this.nameMap.set(p.name, p.fullname);
    }    
